...
class ContactUs(View):
    ...
    def post(self,request:HttpRequest):
        ...
        if form.is_valid():
            if request.user.is_authenticated:
                mesage_contact=MessageToAdmin.objects.create(name=form.cleaned_data.get('name'),title=form.cleaned_data.get('title'),message=form.cleaned_data.get('message'),user_id=request.user.id)
                mesage_contact.save()
        ...
        return render(request,'pages/contact-us.html',{'form':form,'Contact_Info':contact_info,'state':state,'status':status})
